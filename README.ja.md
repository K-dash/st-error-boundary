...
このパターンは本番環境での使用から抽出され、オープンソース化されました。堅牢な Streamlit アプリケーションをコードの明瞭性を犠牲にすることなく構築するためのものです。アーキテクチャの詳細については、[PyConJP 2025 プレゼンテーション](https://speakerdeck.com/kdash/streamlit-hashe-nei-turudakeziyanai-poc-nosu-sadeshi-xian-surushang-yong-pin-zhi-nofen-xi-saas-akitekutiya)を参照してください。

## 機能

- **最小限の API**: 必須引数は 2 つだけ（`on_error` と `fallback`）
- **タイプセーフ**: Python 3.12+ の完全な型ヒントと、厳格な mypy/pyright チェック
- **コールバック保護**: デコレートされた関数とウィジェットコールバック（`on_click`、`on_change` など）の両方を保護
- **プラガブルなフック**: エラー発生時に副作用（監査ログ、メトリクス、通知）を実行
